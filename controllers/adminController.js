  ...
  exports.getBookingsByDate = async (req, res) => {
    try {
        ...
        if (date) {
            ...
            query = { useDate: { $gte: startDate, $lt: endDate } };
        }

        const bookings = await Booking.find(query).sort({ useDate: 1 });
...
exports.getBookingDetails = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Error loading booking details' });
    }
};
...
exports.getVoucherDetails = async (req, res) => {
    try {
      ...
      res.status(500).send('Error loading voucher details');
    }
}  
...
exports.getVouchers = async (req, res) => {
    try {
        ...
        console.log("request userID: ",searchUserId)
        let query = {};
        if (searchUserId) {
            query = { userId: { $regex: searchUserId, $options: 'i' } }; // 변경 사항: query 전체에 할당
        }
