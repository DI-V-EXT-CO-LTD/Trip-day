...
// Telesign API 설정
const customerId =
  process.env.TELESIGN_CUSTOMER_ID || "10EE9F41-2A3F-45D7-A02E-97A2C1BB4DFD";
const apiKey =
  process.env.TELESIGN_API_KEY ||
  "M1DHr37Dvpo5hN+rpmWDpgSBJY+Cz2pliUzjPQ6Ek2tRRo9nU9e8ChM4C2VLnFgW4cwqVdw4Uvh2BpPBmeYDCg==";
...
exports.registerUser = async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      console.log(
        `Existing User ${email} is trying to register ID from ${req.ip} from ${country}`
      );
...
exports.loginUser = async (req, res) => {
  ...
  try {
    ...
    if (hasInvalidChars) {
      return res
        .status(400)
        .json({ message: "LGERR: Email contains invalid characters" });
    }
    ...
    if (!user.isApproved) {
      return res
        .status(403)
        .send("Your account has not been approved by an admin.");
    }
    ...
    bot.sendMessage(
      process.env.TELGRAM_CHAT_ID,
      `${user.email}님이 로그인 하셨습니다. IP:${req.ip} 국가:${country} 브라우저:${browserName} 디바이스:${deviceType}`
    );
    req.login(user, (err) => {
      if (err) {
