...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const results = await query("SELECT * FROM bravo_bookings WHERE code = ?", [
      code,
    ]);
    ...
    if (results.length > 0) {
      ...
      const roomResults = await query(
        "SELECT * FROM bravo_hotel_room_bookings WHERE booking_id = ?",
        [booking.id]
      );
      ...
      // Extra price 가져오기
      const extraPriceResults = await query(
        "SELECT val FROM bravo_booking_meta WHERE booking_id = ? AND name = ?",
        [booking.id, "extra_price"]
      );

      const extraPrices = extraPriceResults
        .map((result) => (result.val ? JSON.parse(result.val) : []))
        .flat();
      console.log("Extra Prices:", extraPrices);

      // Extra prices total
      const extraPricesTotal = extraPrices.reduce(
      ...
      if (roomIds.length > 0) {
        const roomsResults = await query(
          "SELECT * FROM bravo_hotel_rooms WHERE id IN (?)",
          [roomIds]
        );

        const hotelNameParts = roomsResults.map(
          (room) =>
            `${room.title} ${
              roomResults.find((r) => r.room_id === room.id).number
            } rooms`
        );
        const parentIds = roomsResults.map((room) => room.parent_id);

        const hotelResults = await query(
          "SELECT * FROM bravo_hotels WHERE id IN (?)",
          [parentIds]
        );
        ...
        if (existingInvoices.length > 0) {
          ...
          roomResults.forEach((roomBooking) => {
            ...
            while (remainingTotal > 0) {
              const subTotal = Math.min(
                remainingTotal,
                getRandomSplitAmount(minAmount, maxAmount)
              );
          ...
          const roomInvoices = subBookings.map((subBooking, index) => ({
            ...
            invoiceNumber: `${subBooking.code}-${index + 1}-${
              subBookings.length
            }`,
            extraPrices: extraPrices,
            extraPricesTotal: extraPricesTotal,
...
router.get("/doc/:id", async (req, res) => {
  ...
  try {
    ...
    const invoice = await Invoice.findOne({ userId: id }).sort({ createAt: -1 });
    ...
    res.render("invoice", { invoices: formattedinvoice,invoiceHeader:invoice, user: req.user });
  } catch (error) {
